 ...
 *compile:			gcc -lpthread -O2 -o motorx motorx.c
 ...
 */
static pthread_t pthrdsX;//X stand for X Axis
pthread_t pthrdsXmode; //Thread handle for controling the mode of x motor
pthread_t pthrdsXspeed;//Thread handle for controling the mode of x motor
pthread_t pthrdsgetXmode;//Thread handle for get the x motor mode
...
int motor=0;//stand for x axis
int mode[2]={2,0};
int delaytime=1000000;//The delay time of the initial
...
int main(int argc,char *argv[])
{
	...
	/*create fifo file*/
	umask(0);
	if(access("/tmp/xfifo",F_OK)==-1){
		res=mkfifo("/tmp/xfifo",0777);
		if(res==-1){
			printf("create xfifo error\n");
			exit(1);
		}
	}
	if(access("/tmp/xfifomode",F_OK)==-1){
		res=mkfifo("/tmp/xfifomode",0777);
		if(res==-1){
			printf("create xfifomode error\n");
			exit(1);
		}
	}
	if(access("/tmp/xfifospeed",F_OK)==-1){
		res=mkfifo("/tmp/xfifospeed",0777);
		if(res==-1){
			printf("create xfifospeed error\n");
			exit(1);
		}
	}
	printf("if you want to start motor x,please \necho s>/tmp/xfifo\n");
	printf("if you want to stop motor x,please \necho e>/tmp/xfifo\n");

	while(1){
		/*if you like to run the y motor ,you can write s to tell the motor*/
		fd=open("/tmp/xfifo",O_RDONLY);
		if(fd==-1){
			perror("open xfifo error\n");
			exit(1);
		}

		if((read(fd,message,sizeof(char)))==-1){
			printf("In the main function ,read xfifo read error\n");
			exit(1);
		}
		/*'s' stand for start ,it means that start the motor */
		if(message[0]=='s'){
			printf("motor run\n");
			pthread_create(&pthrdsX,NULL,run,NULL);
		}
	...
	unlink("/tmp/xfifo");
	unlink("/tmp/xfifomode");
	unlink("/tmp/xfifospeed");
}

/*run the motor only for x axis
...
static void * run()
{
	...
	/*create a thread to receive speed of x motor*/
	if(pthread_create(&pthrdsXspeed,NULL,(void *)getspeed,NULL)){
		printf("create pthreadXspeed error!\n");
		exit(1);
	}
   /*create a thread to receive the mode of the x motor*/					
	if(pthread_create(&pthrdsgetXmode,NULL,(void *)getmode,NULL)) {
		printf("create pthreadXmod error!\n");
		exit(1);
	}
  /*create a thread to run the motor*/
    if(pthread_create(&pthrdsXmode,NULL,(void *)change_mode,NULL)){
		printf("create pthreadxmode error");
		exit(1);
	}

	/*receive speed from xfifospeed*/
...
/*get the speed of x motor
 *it will change the speed of x motor
...
void getspeed()
{
	...
	int n;
	while(1){
		fd_speed=open("/tmp/xfifospeed",O_RDONLY);
		if(fd_speed==-1){
			printf("open xfifospeed error!\n");
			exit(1);
		}
		if((n=read(fd_speed,speedinfo,10*sizeof(char)))==-1){
			perror("read xfifospeed error!\n");
			exit(1);
		}
		speedinfo[n]='\0';
...
void getmode()
{
	...
	while(1){
		fd_mode=open("/tmp/xfifomode",O_RDONLY);
		if(fd_mode==-1){
			printf("open xfifomode error!\n");
			exit(1);	
		}

		if((read(fd_mode,mode,2*sizeof(char)))==-1){
			printf("in run function read xfifomode error\n");
			exit(1);
		}
		currmode=atoi(mode);
		close(fd_mode);
...
void stop()
{
	pthread_cancel(pthrdsXmode);
	pthread_cancel(pthrdsX);
	pthread_cancel(pthrdsXspeed);
	pthread_cancel(pthrdsgetXmode);
	printf("X motor stoped\n");
}
